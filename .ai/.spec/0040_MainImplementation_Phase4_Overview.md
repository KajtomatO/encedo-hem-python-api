...
**Goal:** Implement `auth_flow.pair()`, `auth_flow.remote_login()`, and supporting helpers.
...
- [ ] `api/auth_flow.py::list_paired()`: `POST /api/auth/ext/mac` (scope `auth:ext:pair`) — obtains `{nonce, mac, eid}` from the device using a broker `epk`, then queries the broker to retrieve the list of paired external authenticators.
...
- `ext/mac` returns `{nonce, mac, eid}` — the `mac` is an authentication credential for the broker to identify the device; `eid` is the device's public key.
